...
export class XxxDetails implements OnInit {
  ...
  loadEntity(paramId: string): void {
    this.xxxService.retrieve(paramId).subscribe(
      (xxx: any)=> {
  ...
  create() {
    this.xxxService.create(this.form.value).subscribe(
      (result: any) => {
        this.router.navigate(['xxx-details', result.id]);
      }
    );
  }

  save() {
    this.xxxService.update(this.xxx.id, this.form.value).subscribe(
      (result) => {
        this.xxx = result;
  ...
  remove() {
    ...
    dialogRef.afterClosed().subscribe(
      (result) => {
        if(result == true) {
           this.xxxService.remove(this.xxx.id).subscribe(
              () => this.backToList()
            );
        }
      }
